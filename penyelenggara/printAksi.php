...
//Sertifikat
$query3="select * from templatesertifikat where IdTemplate = '$ids2'";
...
$ubahstatus = "UPDATE acara SET StatusPrint = 1 WHERE IdAcara = '$ids'";
$statschange =mysqli_query($con,$ubahstatus);
...
//Peserta
$query="select * from peserta WHERE IdAcara = '$ids'";
$fire=mysqli_query($con,$query);
$row=mysqli_fetch_all($fire);
$zipped = array();
...
foreach($row as $x)
{
    ...
    $angle = 45;
    $text_box = imagettfbbox(67,$angle,$font,$text);
    ...
    if (!empty($getrow[6]) && !empty($getrow[7])){
        ...
        imagettftext($image, 26, 0, 680, 850, $color,$font, "diselenggarakan pada tanggal ".$result);
